    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-6">
      <div className="container mx-auto h-20 glass rounded-full flex items-center justify-between px-10 relative">
        ...
        <div className="flex items-center">
          <Link 
            href="/" 
            className="font-headline text-xs md:text-sm font-black tracking-[0.2em] text-foreground uppercase whitespace-nowrap transition-colors hover:text-primary -translate-y-2.5"
          >
            YS OUTFITTERS<span className="text-primary animate-blink text-4xl md:text-5xl leading-none inline-block align-baseline ml-1 drop-shadow-[0_0_20px_rgba(64,138,113,1)]">.</span>
          </Link>
        </div>
          ...
          <div className="relative flex items-center gap-1">
            ...
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                className={cn(
                  "relative z-10 text-[11px] tracking-[0.25em] uppercase transition-colors duration-300 px-4 py-3 rounded-full flex items-center justify-center whitespace-nowrap",
